...
public class ProverbsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_proverbs, container, false);
        //Initialize Firebase DB
        db = FirebaseDatabase.getInstance().getReference();


        //SETUP RECYCLER
        rv = (RecyclerView) rootView.findViewById(R.id.recyclerViewProverbList);
        linearLayoutManager = new LinearLayoutManager(getActivity());
        linearLayoutManager.setStackFromEnd(false);


        progressBarProverbList = (ProgressBar) rootView.findViewById(R.id.progressBarProverbList);
        progressBarProverbList.setVisibility(View.VISIBLE);

        Query query = db.child("Articles").orderByChild("category").equalTo(CATEGORY_PROVERB);
        ...
        firebasenewsRecycleAdapter = new FirebaseRecyclerAdapter<Article, ArticleViewHolder>(Article.class, R.layout.article_item_list, ArticleViewHolder.class, query) {
            ...
            protected void populateViewHolder(ArticleViewHolder viewHolder, final Article model, final int position) {
                ...
                progressBarProverbList.setVisibility(View.GONE);
